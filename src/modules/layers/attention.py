...
class EntityAttentionLayer(nn.Module):
    ...
    def forward(self, entities, pre_mask=None, diff_mask=None, post_mask=None, test_mode=False):
        ...
        if pre_mask is not None:
            ...
            attn_logits = attn_logits.masked_fill(pre_mask_rep[:, :, :ne].bool(), -float('Inf'))
        ...
        attn_weights = attn_weights.masked_fill(attn_weights != attn_weights.bool(), 0)
        ...
        if post_mask is not None:
            ...
            attn_outs = attn_outs.masked_fill(
                post_mask.reshape(bs, n_queries, *[1 for _ in range(n_broadcast_dims)]).bool(), 0
            )
